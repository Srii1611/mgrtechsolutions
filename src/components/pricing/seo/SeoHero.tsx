...
import { SEO_HERO } from '@/data/seo-packages';

/**
 * S1 — SEO packages hero (dark), carrying both the price strip and the
 * ad-spend caveat.
 *
 * Both live in the hero on purpose: the brief makes the prices a
 * without-scrolling requirement, and makes the caveat something the reader
 * must see "at the same moment as the price". Putting either below the
 * hero would let the headline push it off a 375px screen.
 */
export default function SeoHero() {
  const { strip } = SEO_HERO;
    ...
    <section className="dot-grid on-dark bg-forest-950 pb-14 pt-24">
      <div className="container-page">
        <nav aria-label="Breadcrumb" className="eyebrow text-mist">
          ...
          <span className="text-accent">{SEO_HERO.breadcrumb}</span>
        </nav>

        <p className="eyebrow mt-8 text-accent">{SEO_HERO.eyebrow}</p>

        <h1 className="h1-page mt-4 max-w-4xl font-medium text-cream-50">
          {SEO_HERO.headline}
          <span className="mt-1 block text-accent">{SEO_HERO.headlineAccent}</span>
        </h1>

        <p className="lede mt-5 max-w-xl text-mist">{SEO_HERO.lede}</p>
